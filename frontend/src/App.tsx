...
function Developers() {
  ...
  const [tab, setTab] = useState<'keys' | 'webhooks' | 'playground'>('keys')
  ...
  const saveWebhook = useMutation({ mutationFn: async () => editingWebhook ? api.updateWebhook(editingWebhook.id, { url: webhookUrl, events: webhookEvents }) : api.createWebhook({ url: webhookUrl, events: webhookEvents }), onSuccess: data => { if ('secret' in data) setWebhookSecret(data.secret); setShowWebhook(false); setEditingWebhook(null); hooks.refetch() } })
  ...
  return <>
    ...
    <div className="dev-tabs"><button className={tab === 'keys' ? 'active' : ''} onClick={() => setTab('keys')}>API keys</button><button className={tab === 'webhooks' ? 'active' : ''} onClick={() => setTab('webhooks')}>Webhooks</button><button className={tab === 'playground' ? 'active' : ''} onClick={() => setTab('playground')}>API playground</button></div>
