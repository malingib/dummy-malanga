  ...
  try {
    ...
    const onboarding = {
      ...
      mpesa: {
        ...
        accountFormat: String(mpesa.accountFormat || 'invoice'),
      },
      features: {
        sms: Boolean(features.sms),
        email: Boolean(features.email),
        whatsapp: Boolean(features.whatsapp),
        webhook: Boolean(features.webhook),
      },
      webhooks: {
        validation: `${origin}/api/mpesa/validation`,
        confirmation: `${origin}/api/mpesa/confirmation`,
      },
    };

    // Credentials are deliberately not accepted or persisted by this endpoint.
    // Production Daraja secrets belong in server-side environment/secret storage.
    console.info('[onboarding] configured payment workspace', {
      business: onboarding.business.name,
      shortcode: onboarding.mpesa.shortcode,
