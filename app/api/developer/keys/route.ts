...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const scopes = Array.isArray(body?.scopes) ? Array.from(new Set(body.scopes.map((v: unknown) => String(v)))).filter((v) => (API_SCOPES as readonly string[]).includes(v)) : ['payments:read', 'payments:write'];
