...
import { createOrUpdateMpesaConnection, registerMpesaCallbacks, obtainMpesaToken } from '@/lib/mpesa-connection-service'

async function workspaceId() { return (await cookies()).get('mobiwave_workspace_id')?.value || null }

function publicConnection(row: Record<string, unknown>) {
  return {
    id: row.id,
    ...
    token_last_obtained_at: row.token_last_obtained_at,
    callback_registered_at: row.callback_registered_at,
    callback_verified_at: row.callback_verified_at,
...
export async function POST(request: NextRequest) {
  ...
  try {
    const connection = await createOrUpdateMpesaConnection({
      ...
      shortcode,
      accountType: body?.account_type === 'till' ? 'till' : 'paybill',
      environment: body?.environment === 'production' ? 'production' : 'sandbox',
      credentialSource: body?.credential_source === 'mobiwave' ? 'mobiwave' : 'merchant',
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Unable to save M-Pesa connection.' }, { status: 500 })
  }
}
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    if (body?.action === 'verify') {
      ...
      await supabaseAdmin.from('mpesa_connections').update({ is_active: false, connection_status: 'suspended' }).eq('id', connectionId).eq('workspace_id', workspace)
    } else {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'M-Pesa connection operation failed.' }, { status: 502 })
  }
}
