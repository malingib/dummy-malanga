...
function apiBase(environment: Environment) {
  return environment === 'production'
    ? (process.env.MPESA_PRODUCTION_BASE_URL || 'https://api.safaricom.co.ke')
    : (process.env.MPESA_SANDBOX_BASE_URL || 'https://sandbox.safaricom.co.ke')
}

function authUrl(environment: Environment) {
  return `${apiBase(environment)}/oauth/v1/generate?grant_type=client_credentials`
}

function callbackUrl(path: string) {
  const base = (process.env.MOBIWAVE_API_BASE_URL || '').replace(/\/$/, '')
  if (!base) throw new Error('MOBIWAVE_API_BASE_URL is not configured')
  return `${base}${path}`
}

export async function createOrUpdateMpesaConnection(input: ConnectionInput) {
  const environment = input.environment || 'sandbox'
  const accountType = input.accountType || 'paybill'
  const credentialSource = input.credentialSource || 'merchant'

  const encryptedConsumerKey = encryptMpesaSecret(input.consumerKey)
  const encryptedConsumerSecret = encryptMpesaSecret(input.consumerSecret)
  const encryptedPasskey = encryptMpesaSecret(input.passkey)
  ...
  const payload = {
    ...
    ...(encryptedConsumerKey ? { consumer_key_encrypted: encryptedConsumerKey } : {}),
    ...(encryptedConsumerSecret ? { consumer_secret_encrypted: encryptedConsumerSecret } : {}),
    ...(encryptedPasskey ? { passkey_encrypted: encryptedPasskey } : {}),
  ...
  const { data, error } = await supabaseAdmin
    ...
    .upsert(payload, { onConflict: 'workspace_id,shortcode,environment' })
    .select('*')
    .single()
...
export async function getMpesaConnection(connectionId: string) {
  const { data, error } = await supabaseAdmin
    ...
    .maybeSingle()
  if (error) throw new Error(error.message)
  if (!data) throw new Error('M-Pesa connection not found')
  return data
}

export async function obtainMpesaToken(connectionId: string) {
  const connection = await getMpesaConnection(connectionId)
  const consumerKey = decryptMpesaSecret(connection.consumer_key_encrypted)
  const consumerSecret = decryptMpesaSecret(connection.consumer_secret_encrypted)
  if (!consumerKey || !consumerSecret) throw new Error('M-Pesa credentials are required for this connection')

  const basic = Buffer.from(`${consumerKey}:${consumerSecret}`).toString('base64')
  const response = await fetch(authUrl(connection.environment as Environment), {
    method: 'GET',
  ...
  if (!response.ok || !body.access_token) {
    await supabaseAdmin.from('mpesa_connections').update({ token_status: 'failed', connection_status: 'failed', last_error: String(body?.errorMessage || body?.error || 'Unable to obtain M-Pesa access token') }).eq('id', connectionId)
    throw new Error(String(body?.errorMessage || body?.error || 'Unable to obtain M-Pesa access token'))
  }
...
export async function registerMpesaCallbacks(connectionId: string) {
  ...
  const confirmationUrl = callbackUrl('/api/mpesa/c2b/confirmation')
  const validationUrl = callbackUrl('/api/mpesa/c2b/validation')
  ...
  const response = await fetch(url, {
    ...
    body: JSON.stringify({ ShortCode: connection.shortcode, ResponseType: responseType, ConfirmationURL: confirmationUrl, ValidationURL: validationUrl }),
    cache: 'no-store',
  })
  const body = await response.json().catch(() => ({}))
  if (!response.ok || String(body?.ResponseCode || body?.responseCode || '') === '1') {
    const message = String(body?.ResponseDescription || body?.errorMessage || body?.error || 'Safaricom callback registration failed')
    await supabaseAdmin.from('mpesa_callback_registrations').update({ status: 'failed', last_error: message, safaricom_response: body }).eq('id', registration.id)
    await supabaseAdmin.from('mpesa_connections').update({ callback_status: 'failed', connection_status: 'failed', last_error: message }).eq('id', connectionId)
