...
export type ApiAuth = { workspaceId: string; keyId: string; scopes: string[]; actorUserId: string | null };
...
export function createSecret(prefix = 'mw_live_') { const value = `${prefix}${randomBytes(30).toString('base64url')}`; return { value, prefix: value.slice(0, 12), last4: value.slice(-4), hash: hashSecret(value) }; }
...
export async function resolveWorkspace(request: NextRequest): Promise<{ id: string; ownerUserId: string | null } | null> {
  ...
  if (auth?.startsWith('Bearer ')) {
    ...
    if (data.user?.id) {
      ...
      return workspace ? { id: String(workspace.id), ownerUserId: workspace.owner_user_id ? String(workspace.owner_user_id) : null } : null;
    }
  }
...
export async function authenticateApiKey(request: NextRequest, requiredScope?: string): Promise<ApiAuth | NextResponse> {
  ...
  if (error || !key || key.status !== 'active' || (key.expires_at && new Date(key.expires_at) <= new Date()) || !safeEquals(String(key.key_hash), keyHash)) return NextResponse.json({ error: 'Invalid or expired API key.' }, { status: 401 });
  ...
  return { workspaceId: String(key.workspace_id), keyId: String(key.id), scopes: (key.scopes || []).map((scope: unknown) => String(scope)), actorUserId: null };
}
